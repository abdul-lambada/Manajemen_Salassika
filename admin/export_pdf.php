...
try {
    ...
    class PDF extends FPDF {
        function Header() {
            ...
            $this->Cell(0,7,'Periode: ' . date('d-m-Y', strtotime($GLOBALS['tanggal_mulai'])) . ' s/d ' . date('d-m-Y', strtotime($GLOBALS['tanggal_akhir'])),0,1,'C');
            $this->Ln(2);
            // Table header
            $this->SetFont('Arial','B',9);
            $header = array('Nama','Tipe','NIP/NIS','NISN','Kelas','UID','Tanggal','Waktu','Status Manual','Status FP','Mode','Catatan');
            $widths = array(28,13,20,18,18,18,18,15,20,18,15,30);
    ...
    // Jika ada parameter download=1, langsung output PDF
    if (isset($_GET['download']) && $_GET['download'] == '1') {
        ...
        foreach ($laporan_data as $data) {
            $pdf->Cell($widths[0],6,$data['nama_user'],1);
            $pdf->Cell($widths[1],6,ucfirst($data['tipe_user']),1);
            $pdf->Cell($widths[2],6,($data['nip'] ? $data['nip'] : $data['nis']),1);
            $pdf->Cell($widths[3],6,$data['nisn'],1);
            $pdf->Cell($widths[4],6,($data['nama_kelas'] ? $data['nama_kelas'] : '-'),1);
            $pdf->Cell($widths[5],6,$data['fingerprint_uid'],1);
            $pdf->Cell($widths[6],6,($data['tanggal_guru'] ? $data['tanggal_guru'] : $data['tanggal_siswa']),1);
            $pdf->Cell($widths[7],6,($data['waktu_fingerprint'] ? date('H:i:s', strtotime($data['waktu_fingerprint'])) : '-'),1);
            $pdf->Cell($widths[8],6,($data['status_guru'] ? $data['status_guru'] : ($data['status_siswa'] ? $data['status_siswa'] : '-')),1);
            $pdf->Cell($widths[9],6,($data['status_fingerprint'] ? $data['status_fingerprint'] : '-'),1);
            $pdf->Cell($widths[10],6,($data['verification_mode'] ? $data['verification_mode'] : '-'),1);
            $pdf->Cell($widths[11],6,($data['catatan_guru'] ? $data['catatan_guru'] : ($data['catatan_siswa'] ? $data['catatan_siswa'] : '-')),1);
            $pdf->Ln();
        }
    ...
    // Jika tidak, tampilkan halaman dengan tombol download
    ?>
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <div class="container mt-5">
            <div class="card shadow">
                <div class="card-header bg-primary text-white">
                    <h4 class="mb-0">Export PDF Absensi</h4>
                </div>
                <div class="card-body">
                    <p>Data absensi siap diexport ke PDF.</p>
                    <a href="?<?php echo http_build_query(array_merge($_GET, ['download'=>1])); ?>" class="btn btn-success">
